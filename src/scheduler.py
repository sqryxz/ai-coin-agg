...
def daily_summary_job():
    """Job wrapper for generating and saving the daily top coins report."""
    logger.info("Scheduler starting: Daily Summary Report Job")
    try:
        # The generate_and_save_top_coins_report function needs a day_date
        # For a daily report, this would be the current date.
        today = datetime.now(timezone.utc).date()
        
        # If aggregator.py is designed for a daily report, it should handle the date logic.
        # Here, we just call it. The current aggregator.py test uses today.
        generate_and_save_top_coins_report(day_date=today, top_n=config.TOP_N_COINS_REPORT) # Parameter name changed to day_date
        logger.info("Scheduler finished: Daily Summary Report Job completed successfully.")
    except Exception as e:
        logger.error(f"Scheduler error: Daily Summary Report Job failed: {e}", exc_info=True)

if __name__ == "__main__":
    logger.info("--- Scheduler Service Started ---")
    logger.info(f"Main data pipeline will run every {config.HOURLY_PIPELINE_INTERVAL_MINUTES} minute(s).")
    logger.info(f"Daily summary report will run daily at 01:00 UTC.") # Simplified log message
    ...
    schedule.every().day.at("01:00", "UTC").do(daily_summary_job) # Schedule daily at 1 AM UTC
    ...
    # logger.info("Performing initial run of daily_summary_job...")
    # daily_summary_job()
